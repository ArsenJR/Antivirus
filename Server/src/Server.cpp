...
{
	std::u16string filePath = u"Monitors.vaa";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;

	std::u16string header = reader.readU16String();
	if (header != u"Vartanyan")
	{
		reader.close();
		return;
	}
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Vartanyan");
...
{
	std::u16string filePath = u"Scanners.vaa";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.vaa"));
	threats = std::make_shared<ThreatList>(u"Threats.vaa");
	threats->load();
