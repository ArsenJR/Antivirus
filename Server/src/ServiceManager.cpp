...
void ServiceManager::uninstall()
{
	...
	DeleteFile(L"Monitors.vaa");
	DeleteFile(L"Scanners.vaa");
	DeleteFile(L"Threats.vaa");
...
void ServiceManager::createRegistryRecord()
{
	...
	Ret =
		RegCreateKeyEx(
			HKEY_LOCAL_MACHINE,
			TEXT("SOFTWARE\\Vartanyan Anti-Virus"),
			0,
			NULL,
...
{
	HKEY hKey;
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Vartanyan Anti-Virus"),
		0, KEY_READ | KEY_WOW64_64KEY, &hKey) == ERROR_SUCCESS)
	{
		RegCloseKey(hKey);

		RegDeleteTree(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Vartanyan Anti-Virus"));
	}
}
